...
class Emulator(object):
    ...
    def flux(self, params, times, nu):
        ...
        nu_inds = [np.where(nu == nu_obs)[0] for nu_obs in nu_unique]
        f_obs = np.zeros(len(tdata))
        inp_arr = np.zeros((len(nu_unique), self.Nparams))
        inp_arr[:, :] = [(np.log10(E0), np.log10(theta_jn), np.log10(theta_c), np.log10(
            n0), p, np.log10(ee), np.log10(eB), np.log10(nu_obs)) for nu_obs in nu_unique]
        outY_unscaled = 10**((self.NNmodel((inp_arr - self.Xmean) /
                             self.Xstd)) * self.Ystd + self.Ymean).numpy()
        for i, nu_obs in enumerate(nu_unique):
            t_obs = tdata[nu_inds[i]]
            dataOut = np.interp(t_obs, self.tcomp, outY_unscaled[i, :])
