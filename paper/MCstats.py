...
e = np.e
print(
    "Bayes factor Z_wind_scalefit/Z_ism_scalefit : "
    + str((e**lnZ_scalefit_wind) / (e**lnZ_scalefit_ism))
)
print(
    "Bayes factor Z_wind_deepglow/Z_ism_deepglow : "
    + str((e**lnZ_deepglow_wind) / (e**lnZ_deepglow_ism))
)
print(
    "Bayes factor Z_ism_deepglow/Z_ism_scalefit : "
    + str((e**lnZ_deepglow_ism) / (e**lnZ_scalefit_ism))
)
print(
    "Bayes factor Z_wind_deepglow/Z_wind_scalefit : "
    + str((e**lnZ_deepglow_wind) / (e**lnZ_scalefit_wind))
)
...
prmnames_latex = [
    r"$\log_{10}\theta_0$",
    r"$\log_{10}E_{K, \mathrm{iso}}$",
    r"$\log_{10}n_{\mathrm{ref}}$",
    r"$\theta_{\mathrm{obs}} / \theta_0$",
    r"$p$",
    r"$\log_{10}\epsilon_B$",
    r"$\log_{10}\bar{\epsilon}_e$",
    r"$A_V$",
]
paramnames = [
    "theta_0",
...
with open("combined_table_ism.txt", "w") as table:
    ...
    for j, param_name in enumerate(paramnames):
        ...
        if (
            prm_dg_ism.median + prm_dg_ism.upper > prm_sf_ism.median + prm_sf_ism.lower
            and prm_dg_ism.median < prm_sf_ism.median
        ) or (
            prm_dg_ism.median + prm_dg_ism.lower < prm_sf_ism.median + prm_sf_ism.upper
            and prm_dg_ism.median > prm_sf_ism.median
        ):
            line = (
                r"{"
...
with open("combined_table_wind.txt", "w") as table:
    ...
    for j, param_name in enumerate(paramnames):
        ...
        if (
            prm_dg_wind.median + prm_dg_wind.upper
            > prm_sf_wind.median + prm_sf_wind.lower
            and prm_dg_wind.median < prm_sf_wind.median
        ) or (
            prm_dg_wind.median + prm_dg_wind.lower
            < prm_sf_wind.median + prm_sf_wind.upper
            and prm_dg_wind.median > prm_sf_wind.median
        ):
            line = (
                r"{"
                ...
                + r"}"
                + r" & $"
                + "%.2f" % (prm_dg_wind.median)
                + r"^{+"
                + "%.2f" % (prm_dg_wind.upper)
                + r"}_{"
                + "%.2f" % (prm_dg_wind.lower)
                + r"}$"
                + r" & $"
                + "%.2f" % (prm_sf_wind.median)
                + r"^{+"
                + "%.2f" % (prm_sf_wind.upper)
                + r"}_{"
                + "%.2f" % (prm_sf_wind.lower)
                + r"}$ & $ \quad \checkmark$ \\"
                + "\n"
            )
        else:
